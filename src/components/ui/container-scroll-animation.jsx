...
export const ContainerScroll = ({ titleComponent, children }) => {
    ...
    const rotate = useTransform(scrollProgress, [0, 1], [isMobile ? 10 : 20, 0]);
    const scale = useTransform(scrollProgress, [0, 1], scaleDimensions);
    const translate = useTransform(scrollProgress, [0, 1], [0, isMobile ? -35 : -100]);

    return (
        <div
            className="h-[60rem] sm:h-[66rem] md:h-[76rem] flex items-center justify-center relative pt-24 md:pt-28 px-2 md:px-20 pb-2 md:pb-20"
            ref={containerRef}
        >
            <div
...
export const Card = ({ rotate, scale, children }) => {
    return (
        <motion.div
            ...
            className="max-w-5xl -mt-6 sm:-mt-8 md:-mt-12 mx-auto h-[24rem] sm:h-[30rem] md:h-[48rem] w-full border-2 sm:border-4 border-[#6C6C6C] p-2 md:p-6 bg-[#222222] rounded-[22px] md:rounded-[30px] shadow-2xl"
        >
